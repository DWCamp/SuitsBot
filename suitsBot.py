...
description = """SuitsBot v3.2.1
Discord bot deployed by /u/PM_ME_YOUR_MASS to practice webAPI implementation and learn Discord bot scripting in Python. 
Supports a variety of different functions including 
- Call-and-response user tags
- Searching Wikipedia and AniList.co
- Creating and managing lists
- Plaing audio clips in voice chat
"""
...
ERROR_EMBED_COLOR = 11141120
NASA_EMBED_COLOR = 15608125
DEFAULT_EMBED_COLOR = 5120872
LIST_EMBED_COLOR = 51400
YIFF_EMBED_COLOR = 13684944
...
briefHelp = {
	...
	"code" : "Arbitrary code executinon",
...
def printList(printList):
	...
	for element in printList:
		if element is None:
			...
			forReturn += str(element) + ", "
	return forReturn + "]"

async def ping(alert):
	await bot.send_message(devChannel, alert)

async def report(alert, command=None, ctx=None):
	if ctx is None:
		errorEmbed = Embed()
		errorEmbed.title = "Alert"
		await bot.send_message(errorChannel, alert)
		return
	errorEmbed = Embed()
	...
	if command is not None:
		errorEmbed.add_field(name="Command", value=command, inline=False)
	errorEmbed.add_field(name="Message", value=ctx.message.content, inline=False)
	errorEmbed.description = alert
	await bot.send_message(errorChannel, embed=errorEmbed)

async def flag(alert, content=None, ctx=None):
	try:
		if ctx is None:
			await bot.send_message(alertChannel, "Alert:\n" + alert)
			return
		flagEmbed = Embed()
		flagEmbed.title = "Flag"
		flagEmbed.color = 0xFCEF15
		if content is not None:
			flagEmbed.add_field(name="Content", value=content, inline=False)
		flagEmbed.add_field(name="Author", value=ctx.message.author.name, inline=False)
		flagEmbed.add_field(name="Time", value=currTime(), inline=False)
		if ctx.message.channel.is_private:
			flagEmbed.add_field(name="Channel", value="Private", inline=False)
		else:
			flagEmbed.add_field(name="Channel", value=ctx.message.server.name + " / " + ctx.message.channel.name,
								 inline=False)
		flagEmbed.add_field(name="Alert", value=ctx.message.content, inline=False)
		flagEmbed.description = alert
		await bot.send_message(alertChannel, embed=flagEmbed)
	except Exception as e:
		await report(str(e) + "\n\nAlert:\n" + alert, command="Error when producing flag", ctx=ctx)
...
async def getJSON(url, headers=requestHeaders, params={}, contentType=None):
	async with aiohttp.ClientSession(headers=headers) as session:
		async with session.get(url, params=params, json={}) as resp:
			if resp.status is 200:
				json = await resp.json(content_type=contentType)
				return [json, resp.status]	
			return [None, resp.status]

async def postJSON(url, headers=requestHeaders, json={}):
	async with aiohttp.ClientSession(headers=headers) as session:
...
async def getMeowImageURL():
	...
	while json is None and counter < 100:
		[json, status_code] = await getJSON("http://aws.random.cat/meow")
		meowAttempt += 1
		counter += 1
...
def getSubredditInfo(subredditName):
	if subredditName.lower() in mutedSubreddits:
		return None
	resp = requests.get('https://www.reddit.com/r/' + subredditName, headers={'User-agent': 'suitsBot Discord Bot'})
	setCookie = resp.headers['set-cookie']
	if setCookie[:4] != "loid":
		return "http://www.reddit.com/r/" + subredditName;
	else:
		return None
...
def stripHTMLTags(htmlText):
	...
	#Remove the tags and its contents
	for tag in ["small"]:
		tagLoc = htmlText.find("<" + tag)
		i = 0
		while tagLoc > -1 and i < 50:
...
@bot.event
async def on_ready():
	global server, devChannel, alertChannel, errorChannel, nextYiffURL
	print('------------')
	print('Logged in as')
	print(bot.user.name)
	print(bot.user.id)
	server = bot.get_server(CIV_SERVER_ID)
	devChannel = bot.get_channel(DEV_CHANNEL_ID)
	alertChannel = bot.get_channel(ALERT_CHANNEL_ID)
	errorChannel = bot.get_channel(ERROR_CHANNEL_ID)
	print('------------')
	print('Awaiting commands...')
	print('------------')
	try:
		await bot.change_presence(game=discord.Game(name='with bytes'))
	except:
		pass
	load()
	bot.player = None
	...
	await bot.send_message(devChannel, embed=readyEmbed)
	bot.close()
...
async def on_message(message):
	...
	if message.content == "!r":
		if message.author.id == DEV_AUTHOR_ID:
			closeConnector()
	...
	if authorID not in users.keys() and failedToLoadUsers is None:
		await report("Added new user " + str(authorID) + ":" + message.author.name)
	...
	if lowerContent.find(" /r/") >= 0:  # Searching for a subreddit mention
		start = message.content.find(" /r/") + 4
	elif lowerContent.find(" r/") >= 0:
		start = message.content.find(" r/") + 3
	elif lowerContent[0:3] == "/r/":
		start = 3
	elif lowerContent[0:2] == "r/":
		start = 2

	if start >= 0:
		end = start
		# Iterate over characters until it finds the end of the subreddit name
		while end < len(message.content) and (message.content[end].isalnum() or message.content[end] == "_"):
			end += 1
		if start is not end:  # If there was a subreddit name
			subName = getSubredditInfo(message.content[start:end])
			if subName is not None:
				await bot.send_message(message.channel, subName)
	...
	try:
		...
		await report(str(e), command="swear detection", ctx=ctx)
		return
...
async def on_voice_state_update(before, after):
	try:
		...
		await report(str(e), command="aes command", ctx=ctx)
		return
...
# Bot interfaces with MyAnimeList.net's API
@bot.command(pass_context=True, help=longHelp['anime'], brief=briefHelp['anime'], aliases=aliases['anime'], hidden=True)
async def anime(ctx):
...
'''
	...
	try:
		...
		if "char" in commands:
			...
			[json, status] = await postJSON(ANILIST_API_URL, json={'query': characterQuery, 'variables': characterVariables})
			if "json" in commands:
			...
			[json, status] = await postJSON(ANILIST_API_URL, json={'query': animeQuery, 'variables': animeVariables})
			if "json" in commands:
				await bot.say(trimToLength(json, 2000))
			if status == 200:
		...
		if status != 200:
			if status == 500:
				await flag("500 Server Error on search term " + searchTerm, content=str(json), ctx=ctx)
				await bot.say("`500 Server Error`. The AniList servers had a brief hiccup. Try again in a little bit")
			elif status == 404:
				await flag("Failed to find result for search term " + searchTerm, content=str(json), ctx=ctx)
				await bot.say("I found no results for `" + searchTerm + "`")
			elif status == 400:
				await report(str(json), command="Error in `!anime` search for term " + searchTerm, ctx=ctx)
				await bot.say("The bot made an error. My bad. A bug report has been automatically submitted")
			else:
				await report(str(json), command="Unknown Error Type", ctx=ctx)
		...
		await report(str(e), command="!anime command", ctx=ctx)
...
async def bestGirl(ctx):
	...
	commandDict = {"!bestGirl": "Presents your list",
			   "!bestGirl updating": "Presents your list that will update after any future changes",
...
async def code(ctx):
	...
	try:
		...
		if "dev" in commands:
			[json, responseCode] = await postJSON(url=creditsCheckURL, json={
				"clientId": JDOODLE_ID, 
			...
			if "```" in trimmedMessage:
				...
				if trimmedMessage[0] not in [" ", "\n"] and len(splitMessage) > 1:
					...
					if language.lower() in langs.keys():
						response = await postJSON(url=executeURL, json={
							"clientId": JDOODLE_ID, 
							"clientSecret": JDOODLE_SECRET,
							"script": code,
		...
		await report(str(e), command="!code command", ctx=ctx)
...
@bot.command(pass_context=True, help=longHelp['jpeg'], brief=briefHelp['jpeg'], aliases=aliases['jpeg'])
async def jpeg(ctx):
	headers = {"Content-Type": "multipart/form-data; boundary=---------------------------17771278719678"}
	payloadPadding = """-----------------------------17771278719678
...
"""
	if len(ctx.message.attachments) > 0:
		...
		# removes the "!jpeg" envocation portion of the message
		commandText = stripCommand(ctx.message.content.strip())
		commandItems = commandText.split(" ")
		...
		if url == "":
			await bot.say("I didn't see a url or file to compress")
			return

	r = requests.post("http://needsmorejpeg.com/upload", headers=headers, data=payloadPadding + url)
	if r.status_code == 200:
		returnURL = "http://static." + r.url[7:] + ".jpg"
		lastJpegImage = returnURL
		embededJpeg = Embed().set_image(url=returnURL)
		await bot.say(embed=embededJpeg)
	else:
		await bot.say("Something went wrong. Make sure the text you give me is a valid image url")
...
async def ls(ctx):
	...
	commandDict = {"!list": "Will print the current list if there is one",
						   ...
						   "title [<id?>] <title>": "Assigns the title to the list with that index",
...
async def meow(ctx):
	...
	try:
		...
		nextMeowURL = None
		embededCat.color = 0xFCBE41
		await bot.say(embed=embededCat)
		await getMeowImageURL()
	except Exception as e:
		await report(str(e), command="Meow command", ctx=ctx)
...
async def nasa():
	try:
		[json, status_code] = await getJSON(
		...
		await report(str(e), command="NASA APOD command", ctx=ctx)
...
async def preferences(ctx):
	...
	if ctx.message.author.id != DEV_AUTHOR_ID:
		await bot.say("This command is restricted to dev use only")
		return

	# removes the "!preferences" envocation portion of the message
	message = stripCommand(ctx.message.content)
	(function, space, parameter) = message.partition(" ")
...
async def tag(ctx):
	...
	try:
		...
		await report(str(e), command="Tag command", ctx=ctx)
...
async def wiki(ctx):
	...
	async def searchForTerm(searchTerm):
		...
		wikiSearchJSON = await getJSON(wikiSearchURL) # Looks for articles matching the search term
	...
	async def queryArticle(articleTitle):
		...
		response = await getJSON(wikiQueryURL) # Gets the article details
		...
		if "extract" not in response.keys(): #If the returned result isn't usable
			await report(trimToLength(response, 2000), command="Wiki command found no useful article", ctx=ctx) #Figure out why it's not usable
			return None
	...
	async def parseSections(articleTitle):
		...
		response = await getJSON(wikiParseURL) # Gets the article details
	...
	try:
		...
		if "help" in commands:  # provides help using this command
			...
			commandDict = {
				...
				"-sections <title>" : "Displays the seaction titles for the article",
		...
		if "sections" in commands:
			if section is None:
				...
				if extractSection is None:
					...
					wikiEmbed.color = 0xFFFFFF # Make the embed white
		...
		await report(str(e), command="Wiki command", ctx=ctx)
...
async def wolf(ctx):
	try:
		...
		await report(str(e), command="Wolf command", ctx=ctx)
...
async def woof(ctx):
	# await bot.say("I don't do that anymore")
	try:
		json = await getJSON("https://dog.ceo/api/breeds/image/random")
		if 'status' not in json[0].keys() or json[0]['status'] != "success":
			...
			await flag("Error with random dog api", content=json, ctx=ctx)
			return
		embededDog = Embed().set_image(url=json[0]['message'])
		embededDog.color = 0xD3D3D3
		await bot.say(embed=embededDog)
	except Exception as e:
		await report(str(e), command="Woof command", ctx=ctx)
...
async def yiff(ctx):
	...
	memeQuotes = ["glom", "OwO", "*notices bulge*", "What's this?",
				  ...
				  "Don't kinkshame me", "furries > scalies",
	...
	try:
		...
		if nextYiffURL is None:
			imageURL = await getYiffImageURL()
		else:
			imageURL = nextYiffURL
			nextYiffURL = None
		embededPhoto = Embed().set_image(url=imageURL)
		...
		nextYiffURL = await getYiffImageURL()
	except Exception as e:
		await report(str(e), command="Yiff command", ctx=ctx)
...
async def say(ctx):
	...
	try:
		...
		if "help" in commands:  # provides help using this command
			...
			await bot.say("`!say` User Guide", embed=dictToEmbed(commandDict))
			return
		if "ls" in commands:
		...
		await report(str(e), command="Say command", ctx=ctx)
...
async def dev(ctx):
	try:
		if ctx.invoked_subcommand is None:
			...
			await bot.say("`!dev` User Guide", embed=dictToEmbed(helpDict))
	except Exception as e:
		await report(str(e), command="dev command", ctx=ctx)
...
async def channelID(ctx):
	try:
		...
		await report(str(e), command="dev channelID command", ctx=ctx)
...
async def meow():
	try:
		...
		await report(str(e), command="dev meow command", ctx=ctx)
...
async def listHelper(ctx, helpEmbed=None, command="list", listID=None):
	...
	# removes the "!list" envocation portion of the message
	message = stripCommand(parsedCTX)
	spaceLoc = message.find(" ")
	...
	if function in ["", "updating"]:
		try:
			...
			await report(str(e), command="List updating command, via " + command, ctx=ctx)
		return
	...
	if function == "static":
		try:
			...
			await report(str(e), command="List static command, via " + command, ctx=ctx)
		return

	# --------------------- EDIT FUNCTIONS

	closedForDevWork = False
	if closedForDevWork and ctx.message.author.id != DEV_AUTHOR_ID:
		await bot.say(
	...
	if function in ["add", "insert"]:  # X
		try:
			...
			await report(str(e), command="List add command, via " + command, ctx=ctx)
		return
	...
	if function == "clear":  # X
		try:
			...
			await report(str(e), command="List clear command, via " + command, ctx=ctx)
		return
	...
	if function in ["curr", "currlist"]:
		try:
			...
			await report(str(e), command="List curr command, via " + command, ctx=ctx)
		return
	...
	if function == "create":  # X
		try:
			...
			await report(str(e), command="List create command, via " + command, ctx=ctx)
		return
	...
	if function == "dev":
		try:
			...
			await report(str(e), command="List dev command", ctx=ctx)
		return
		...
		try:
			...
			if listID == "":  # If the user did not provide a list, use the currently active list
				...
				if listID not in authorLists.keys():  # If the user provided ID doesn't exist, reject the command
					...
					return
			if listSpaces[authorID] == authorLists[
				listID]:  # If that list was the active list, set the active list to none
				listSpaces[authorID] = None
			...
			await report(str(e), command="List drop command, via " + command, ctx=ctx)
		return
	...
	if function == "move":
		try:
			...
			await report(str(e), command="List move command, via " + command, ctx=ctx)
		return
	...
	if function == "multiadd":
		try:
			...
			await report(str(e), command="List multiadd command, via " + command, ctx=ctx)
		return
	...
	if function in ["remove", "delete"]:
		try:
			...
			await report(str(e), command="List remove command, via " + command, ctx=ctx)
		return
	...
	if function in ["replace", "rename", "edit"]:
		try:
			...
			await report(str(e), command="List replace command, via " + command, ctx=ctx)
		return
	...
	if function == "show":
		try:
			...
			await report(str(e), command="List show command, via " + command, ctx=ctx)
		return
	...
	if function == "swap":
		try:
			...
			await report(str(e), command="List swap command, via " + command, ctx=ctx)
		return
	...
	if function == "thumbnail" or function == "icon":
		try:
			...
			await report(str(e), command="List thumbnail command, via " + command, ctx=ctx)
		return
	...
	if function == "title":
		try:
			if len(parameter) > 0 and parameter[0] == "[":  # Makes sure there is a 0th character then checks for an ID
				closeBracketLoc = parameter.find("]")  # Find the close of the ID tag
				if closeBracketLoc == -1:  # If the tag didn't close, reject the command
					await bot.say("You never closed that bracket")
					return
				listID = parameter[1:closeBracketLoc].lower()  # Grab the ID
				title = parameter[closeBracketLoc + 1:].strip()  # Save everything after the close bracket as the title
				if listID not in authorLists.keys():  # If there is no list for the ID found, reject the command
					await bot.say(
						"You do not have a list with the ID '" + listID + "'. Type `!list show` to see your table of list IDs")
					return
				targetList = authorLists[listID]  # Save the list as the target of the command
			else:  # If there was no ID provided, use the currently active list
				if currList is None:  # If there is no currently active list, reject the command
					await bot.say(
						"You are not currently in any list. Type `!list use <id>` to begin editing a list or `!list help` for more information")
					return
				...
				targetList = currList
				listID = currList.id
			targetList.title = title  # Set the title
			updateListDetails(authorID, listID)  # Save to database
			if currList is not None:
				await currList.update()
			await bot.say(
				"Alright. I have changed the title of the list with ID `` " + targetList.id + " `` to `` " + title + " ``")
		except Exception as e:
			await report(str(e), command="List title command, via " + command, ctx=ctx)
		return
	...
	if function == "use":
		try:
			...
			await report(str(e), command="List use command, via " + command, ctx=ctx)
		return
	...
	if function == "help":
		try:
			if helpEmbed is not None:
				...
				await report("Found no helpEmbed for " + command, command=command, ctx=ctx)
			...
			await report(str(e), command="List help command, via " + command, ctx=ctx)
		return
...
def loadCache():
	global failedToLoadCache, nextYiffURL, nextMeowURL, meowSuccess, meowAttempt
	try:
...
CLIENT_ID, CLIENT_SECRET, BOT_TOKEN, MYSQL_USER, MYSQL_PASSWORD, WOLFRAMALPHA_APPID, JDOODLE_ID, JDOODLE_SECRET = tokenFile.read().splitlines()
